        ...
        viewModel.getActivities(criteria, if (speciality.isNotNullable())
            ...
            runOnUiThread(Runnable {
                ...
                with(childFragmentManager.fragments) {
                    (firstOrNull() { it is HCLMapResultFragment } as? HCLMapResultFragment)?.updateActivities(activities)
                    (firstOrNull() { it is HCLListResultFragment } as? HCLListResultFragment)?.updateActivities(activities)
                }
            })
        }
    }
