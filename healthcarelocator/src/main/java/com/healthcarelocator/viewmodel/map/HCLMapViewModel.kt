...
class HCLMapViewModel : AppViewModel<MapFragment>() {

    /**
     * Get 4 coordinates to create a bounding box
     * The result must be an [Array] [North, East, South, West]
     */
    fun getOSMBoundLevel(map: MapView, markers: ArrayList<HCLMarker>) {
        disposable?.add(
    ...
    fun getGoogleMapBoundLevel(map: GoogleMap, markers: ArrayList<HCLMarker>) {
        disposable?.add(
                Flowable.just(markers)
                        .map {
                            val latSorted = ArrayList(it).apply {
    ...
    fun getOSMBoundNearMeLevel(className: String, context: Context, map: MapView, markers: ArrayList<HCLMarker>) {
        ...
        client.requestLastLocation().registerDataCallBack({ currentLocation ->
            ...
            disposable?.add(Flowable.just(markers)
                    .map {
                        ...
                        if (className == HCLNearMeFragment::class.java.simpleName)
    ...
    fun getGoogleMapBoundNearMeLevel(context: Context, map: GoogleMap, markers: ArrayList<HCLMarker>) {
        val client = LocationClient(context)
        client.requestLastLocation().registerDataCallBack({ currentLocation ->
            client.removeLocationUpdate()
            client.releaseApiClient()
